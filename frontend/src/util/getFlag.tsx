...
export function getFlag(currCountry: string, validCurFlagList: string[]): React.JSX.Element {
...
const isValidParam = (currCountry: string): boolean => {
...
const getBaseUrl = (currCountry: string): string => {
...
export async function fetchAllCountryFlags(): Promise<string[]> {
    try {
        ...
        $('section.Country').each((index, element) => {
            const countryCode: string = $(element).find('h1').text().trim();
...
const style = {
    ...
    divGetFlag: { fontSize: '0.6rem', textAlign: 'center' as const, padding: '5.5px 0px', fontWeight: 'bolder' },
}